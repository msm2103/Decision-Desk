...
export function SubscribeForm({ compact = false }: SubscribeFormProps) {
  ...
  return (
    <form method="post" action="/api/subscribe" onSubmit={handleSubmit} className="space-y-3">
      ...
      {!compact ? (
        <p className="text-xs text-slate-500">
          You will receive an email when a new note is published. Unsubscribe at any time from any email or via the unsubscribe page.
        </p>
      ) : null}
    </form>
  );
